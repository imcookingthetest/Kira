...
def analyze_screen_with_vision(image: Image.Image, user_command: str, conversation_context: str = "") -> dict:
    """
    Analyze screen for actions. Now uses OCR + pattern matching as primary method.
    Vision model used only as fallback.
    """
    
    # Check if this is a click command
    if any(word in user_command.lower() for word in ['click', 'clic', 'apri', 'link']):
        print("DEBUG: Detected click command, using OCR+color detection")
        return find_link_by_position(user_command, image)
    
    # For other commands (scroll, type), use vision model
...
def analyze_with_vision_model(image: Image.Image, user_command: str, conversation_context: str = "") -> dict:
    """
    Fallback: Use vision model for non-click actions or when OCR fails.
    Now with MUCH shorter, clearer prompt.
    """
